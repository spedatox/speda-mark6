...
- Run shell commands in an isolated, compact Kali sandbox with a persistent workspace — Python, Node.js, Go, git, build tools, and data/document tooling are ready for real computation; agents can add task-specific packages when needed.
...
- **Sandbox** — a no-secrets compact Kali container that executes arbitrary shell commands with a hard timeout, giving the assistant a real, stateful development workspace without touching the API container or its credentials.
...
| Code execution | Isolated compact Kali sandbox container |
