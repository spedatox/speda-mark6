...
function ProjectsNav({ config, active, onOpen }: {
  ...
  return (
    <div style={{ padding: '0 0.4rem 4px' }}>
      <button
        onClick={() => onOpen()}
        onMouseEnter={() => setHover(true)}
        onMouseLeave={() => setHover(false)}
...
function SidebarHeader({ profile, onToggle, onSearch, searchActive }: {
  ...
  return (
    <div className="hb-seam-b" style={{
      ...
      {/* Brand — single agent, no switcher */}
      <div
        style={{
          display: 'flex', alignItems: 'center', gap: '0.4rem',
...
export default function Sidebar({ profile, config, isOpen, mobile, onSelectSession, onToggle, onNewChat, onOpenSettings, onOpenProjects, projectsOpen }: Props) {
