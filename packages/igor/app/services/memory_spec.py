...
SECTION_COLLECTIONS: tuple[CollectionSpec, ...] = (
    ...
    CollectionSpec(
        ...
        members=(
            ...
            MemberSpec(stem="legacy", title="Unreconciled Financial History", shard=True,
                       index_pattern=r"^\d{4}-\d{2}$", index_level=1, key_shape="YYYY-MM",
                       summary="Unreconciled historical source tables; not live balances or additive transactions",
                       shard_max_bytes=24000),
            MemberSpec(stem="records", title="Financial Records", shard=True,
                       index_pattern=r"^[a-z0-9]+(?:-[a-z0-9]+)*$", index_level=1,
                       key_shape="stable-record-id", summary="Typed, evidenced financial records; use finance_record",
                       shard_max_bytes=24000),
            MemberSpec(stem="balances", title="Account Balances", summary="Computed dated account balance observations"),
            MemberSpec(stem="reports", title="Credit Reports", summary="Computed source report index; never debt rows"),
            MemberSpec(stem="monthly-structure", title="Monthly Structure",
                       takes=("Monthly structure",),
                       summary="Computed recurring rules only; never actual monthly transactions or balances"),
