...
ROUTES = (
    ...
    Route("event", "subject's dated log under <category>/<MM-YY>/; general/ when no specialist domain owns it", "ledger_append / registry_upsert",
          "Something that happened. Keep the date and outcome. Never promote an action to active context merely because it is recent."),
    Route("experience", "/memories/general/<MM-YY>/<topic>.md", "memory_event",
          "A meaningful personal experience, event, household change, or reference without a specialist domain. "
          "Capture what the owner reported — even ordinary trips, outings, and milestones. "
...
def protected_write(path: str, author: str, *, managed: bool = False) -> str | None:
    ...
    if "/.views/" in path:
        return "Views are generated by the system and are not directly editable."
    if path.startswith("/memories/finance/ledger/") or path == "/memories/finance/monthly-structure.md" or re.match(r"^/memories/finance/\d{2}-\d{2}/.*(?:ledger|balances|reports)\.md$", path):
...
def route_for_kind(kind: str) -> Route | None:
    """Find the first Route matching the given kind."""
    for route in ROUTES:
        if route.kind == kind:
            return route
    return None
