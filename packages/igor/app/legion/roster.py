...
def build_tool_definition() -> dict:
    ...
    return {
        ...
        "description": (
            ...
            "`archivist` when the owner's own history needs many searches. Do not use "
            "research workers for news, quick facts, reminders, or work completable "
            "with a handful of direct tool calls. One `search_memory` or "
            "`recall_conversations` call answers most memory questions and is always "
            "the first move; the archivist is for when several would not. "
