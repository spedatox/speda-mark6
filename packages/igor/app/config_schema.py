...
CONFIG_GROUPS: list[ConfigGroup] = [
    ...
    ConfigGroup(
        ...
        [
            ...
            ConfigField("ace_enabled", "Adaptive Countermeasure Engine", "bool",
                        requires_restart=_LIVE,
                        help="Maintain evidence-scored patterns and inject only the tactical "
                             "patterns relevant to the current objective."),
            ConfigField("ace_tactical_max_patterns", "Tactical Pattern Limit", "int",
                        requires_restart=_LIVE,
                        help="Maximum relevant patterns placed in one turn's Tactical Context."),
            ConfigField("ace_tactical_max_countermeasures", "Tactical Countermeasure Limit", "int",
                        requires_restart=_LIVE,
                        help="Maximum proven or candidate strategies placed in Tactical Context."),
            ConfigField("ace_tactical_max_chars", "Tactical Context Character Cap", "int",
                        requires_restart=_LIVE,
                        help="Hard ceiling for the per-turn ACE block."),
            ConfigField("ace_min_query_chars", "Minimum Tactical Query Length", "int",
                        requires_restart=_LIVE,
                        help="Messages shorter than this do not trigger tactical retrieval."),
            ConfigField("ace_active_min_support", "Pattern Activation Evidence", "int",
                        requires_restart=_LIVE,
                        help="Supporting evidence items required before a pattern may become active."),
            ConfigField("ace_active_min_diversity", "Pattern Activation Diversity", "int",
                        requires_restart=_LIVE,
                        help="Independent source groups required before a pattern may become active."),
    ...
    ConfigGroup(
        ...
        [
            ...
            ConfigField("forge_cell_backend", "Forge Cell Backend", "select",
                        options=["docker", "subprocess", "auto"],
                        help="Isolation backend for Forge workers. Use Docker for untrusted code."),
            ConfigField("forge_workspace_root", "Forge Workspace Root", "text",
                        help="Allowed host-visible root for Forge workspaces. Hisar paths under /Forge/workspaces are mapped here.",
                        placeholder="/opt/hisar/vault/Forge/workspaces"),
            ConfigField("forge_coder_image", "Forge Coder Image", "text",
                        help="Docker image used by anonymous coding workers.",
                        placeholder="forge-cell-optimus:latest"),
            ConfigField("forge_reviewer_image", "Forge Reviewer Image", "text",
                        help="Docker image used by anonymous review workers.",
                        placeholder="forge-cell-optimus:latest"),
            ConfigField("forge_pentester_image", "Forge Pentester Image", "text",
                        help="Docker image used by anonymous security workers.",
                        placeholder="forge-cell-centurion:latest"),
            ConfigField("forge_worker_max_iterations", "Forge Worker Iterations", "int",
                        requires_restart=_LIVE,
                        help="Maximum model/tool rounds for one anonymous Forge worker."),
            ConfigField("forge_worker_timeout_s", "Forge Command Timeout (s)", "int",
                        requires_restart=_LIVE,
                        help="Default wall-clock limit for each command inside a Forge Cell."),
