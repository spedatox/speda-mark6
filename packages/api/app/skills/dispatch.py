...
class HousePartySkill(Skill):
    ...
    description = (
        ...
        "FIRST render the warning card by emitting a fenced code block whose info "
        "string is EXACTLY `hpp-warning` (three backticks, then hpp-warning) — not "
        "`hpp`, not `text`. The UI draws the whole heavy/expensive/prototype "
        "authorization window itself, so leave the block body EMPTY except for an "
        "optional single `objective: <one line>` when you know the mission. Do not "
        "write the warning text yourself. After the block, ask the owner to speak "
        "the authorization passphrase; only once they give it in their message do "
        "you call this tool with that exact passphrase. Engage "
