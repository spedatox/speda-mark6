...
- **Five in-process profiles + one external peer.** Sentinel (finance), NightCrawler (OSINT/research), Ultron (academic research/synthesis), Centurion (cyber security), Atomix (the owner's personal health) are profile + prompt-directory pairs inside this repo, addressed via `/chat/{agent_id}` and the existing `/trigger/{agent_id}`. **Optimus is NOT a profile** — it is a standalone framework (system ops/code/infrastructure) that connects back as an external WebSocket peer (OQ4).
