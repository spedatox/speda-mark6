...
- `end_turn` — Codex is done. Return response to user.
...
Agent name, personality, system prompt template, tool allowlist, and model policy live in `app/profiles/{agent_id}.py`. The engine is untouched by identity. Model IDs (`Codex-sonnet-4-6`, `Codex-haiku-4-5-20251001`) live exclusively in individual profile files under `app/profiles/`. They must not appear in `config.py`, `orchestrator.py`, or any core module.
...
Codex sees all four tiers identically in the tools array. The registry is the only entity that knows the difference.
...
| Context | Model |
|---------|-------|
| User-facing interactive response | Codex-sonnet-4-6 |
| Background monitoring, pre-filter, classification | Codex-haiku-4-5-20251001 |
| Agent-to-agent subtasks | Codex-haiku-4-5-20251001 (Sonnet if complexity demands) |
| House Party Protocol (engaged) | Codex-sonnet-4-6 across all agents — full interactive grade, not the background tier |
